...
use aes_gcm::KeyInit;
use axum::{
    extract::DefaultBodyLimit, http::{HeaderValue, Method},
...
use server::{
    api::{
        members::routes::{
            ...
            edit_member, export_members, get_member_invites, get_members,
            get_members_flat, invite_member, request_add_member, upload_members_csv,
        },
        sessions::refresh_session,
        users::routes::{
            accept_member_invite, admin_login, decline_member_invite, logout, me,
            member_login, verify_totp,
        },
    },
    pages::{
        add_request_page, admin_login_page, admin_page, admin_register_page,
        invite_reply_page, members_login_page, user_page,
    },
...
async fn main() {
    ...
    let pool = PgPool::connect(
            &std::env::var("DATABASE_URL")
                .expect(
                    "DATABASE_URL should be defined, example: postgres://postgres:shajarah-dev@localhost:5445/postgres",
                ),
        )
        .await
        ...
        Err(err) => {
            match &err {
                ConfigError::IoError(
                    ...
                    let totp_encryption_key = aes_gcm::Aes256Gcm::generate_key(
                            rand::thread_rng(),
                        )
                        .to_vec();
                    let config = Config {
                    ...
                    let config_str = toml::to_string(&config)
                        .expect("Serialize config struct to toml string");
    ...
    let (email_sender, mut email_receiver) = tokio::sync::mpsc::channel::<
        EmailMessage,
    >(10);
        ...
        tokio::spawn(async move {
            ...
            while let Some(email_message) = email_receiver.recv().await {
                let Ok(m) = Message::builder()
                    ...
                    .body(email_message.content) else {
    ...
    let totp_encryption_key = aes_gcm::Key::<
        aes_gcm::Aes256Gcm,
    >::from_exact_iter(config.totp_encryption_key)
    ...
    let mut app = Router::new()
        ...
        .route("/api/members/disapprove/:id", put(disapprove_member_request))
    ...
    let app = app
        .layer(
            CorsLayer::new()
                .allow_origin([
                    "https://shajarah.bksalman.com".parse::<HeaderValue>().unwrap(),
                ])
                .allow_methods([Method::GET, Method::POST, Method::PUT, Method::DELETE]),
        )
        .layer(axum::middleware::from_fn_with_state(app_state.clone(), refresh_session))
