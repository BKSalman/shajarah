...
use crate::{
    ...
    auth::AuthExtractor, InnerAppState,
};
...
WHERE sessions.id = $1
            "#,
            Uuid::parse_str(session_id.value()).map_err(| e | { log::error!("{e}");
...
WHERE users.email = $1
    ...
    if argon2.verify_password(payload.password.as_bytes(), &parsed_password).is_err() {
        return Err(UsersError::InvalidCredentials);
    }
    let now = Utc::now();
...
RETURNING sessions.id
        "#,
        new_session.id, new_session.user_id, new_session.created_at, new_session
        .expires_at,
    )
        .fetch_one(&mut *tx)
        .await?;
...
WHERE users.email = $1
    ...
    let decrypted_totp_secret = cipher
        .decrypt(
            nonce.try_into().map_err(|_e| UsersError::InternalServerError)?,
    ...
    let totp = totp_rs::TOTP::new(
        ...
        None,
        format!("totp@{}", state.base_url.host().ok_or(UsersError::InternalServerError)?),
    )?;
...
DELETE FROM sessions WHERE sessions.id = $1
        ...
        .await?;
    let cookie = Cookie::build((SESSION_COOKIE_NAME, "")).path("/").http_only(true);
    #[cfg(not(debug_assertions))]
    let cookie = cookie.secure(true);
...
WHERE role = $1
    ...
    let user = sqlx::query_as!(
        ...
        "#,
        Uuid::new_v4(), payload.first_name, payload.last_name, payload.email,
        hashed_password, UserRole::Admin as _, Utc::now(),
    )
...
pub async fn accept_member_invite(
    ...
    sqlx::query!(
        r#"
            ...
            WHERE id = $2
        "#,
        encrypted_totp_secret_with_nonce, invitation_id,
    )
        .execute(&mut *tx)
        .await?;
    let totp = totp_rs::TOTP::new(
...
WHERE id = $2;
"#,
        InviteStatus::Accepted as _, invite_id,
    )
        .execute(&state.db_pool)
        .await?;
...
pub async fn verify_totp(
    ...
    VALUES ($1, $2, $3, $4, $5, $6, $7)
            "#,
        Uuid::new_v4(), member.name, member.last_name, member_invite.email,
        encrypted_totp_secret, UserRole::User as _, Utc::now(),
    )
        .execute(&mut *tx)
        .await?;
    tx.commit().await?;
