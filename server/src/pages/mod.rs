...
pub async fn admin_page(
    ...
    match auth {
        Ok(auth) => {
            ...
            let Json(members) = match get_members_flat(
                    state.clone(),
                    Query(params.members_params),
                )
                .await
            {
                Ok(members) => members,
            ...
            let Json(add_requests) = get_requested_members_flat(
                    state.clone(),
                    Query(params.requests_params),
                )
                .await?;
            let name = auth.current_user.first_name.clone();
            let Json(member_invites) = get_member_invites(
                    auth,
                    state,
                    Query(params.invite_params),
                )
                .await?;
            Ok(
                AdminTemplate {
