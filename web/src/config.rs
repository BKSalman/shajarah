...
pub struct EmailCredentials {
    pub username: String,
    #[serde(skip_serializing)] // Don't accidentally log passwords
    pub password: String,
}
...
pub struct Config {
    // Store as base64 string in TOML, convert to Vec<u8> internally
...
// Custom serialization for the encryption key
...
impl Config {
    ...
    fn apply_email_env_overrides(&mut self) -> Result<(), ConfigError> {
        ...
        // If any email env vars are set, create or update email config
    ...
    fn validate(&self) -> Result<(), ConfigError> {
        ...
        // TOTP keys are typically 32 bytes (256 bits)
...
impl Default for Config {
    fn default() -> Self {
        Self {
            cookie_secret: Vec::new(),       // Will be required from env
            totp_encryption_key: Vec::new(), // Will be required from env
            email_config: None,
...
mod tests {
    ...
    fn test_validation() {
        let mut config = Config {
            cookie_secret: vec![1, 2, 3],       // Too short
            totp_encryption_key: vec![1, 2, 3], // Too short
            email_config: None,
